...
import com.api.reserva.entity.ReservaConvidados;
...
import com.api.reserva.repository.ReservaConvidadosRepository;
...
public class ReservaService {
    ...
    @Autowired
    private ReservaConvidadosRepository reservaConvidadosRepository;
    ...
    public List<ReservaReferenciaDTO> buscarPorUsuario(Authentication authentication) {
        ...
        return reservaRepository.findAll()
                .stream()
                .filter(reserva -> reserva.getHost().getId().equals(usuarioId) ||
                        reserva.getConvidados().stream()
                                .anyMatch(conv -> conv.getConvidado().getId().equals(usuarioId)))
                .map(ReservaReferenciaDTO::new)
                .toList();
    }
    ...
    private void adicionarConvidados(Reserva reserva, Set<Long> convidadosIds) {
        Set<ReservaConvidados> convidados = new HashSet<>();
    ...
    private void atualizarConvidados(Reserva reserva, Set<Long> novosConvidadosIds) {
        ...
        if (reserva.getConvidados() != null) {
            reservaConvidadosRepository.deleteAll(reserva.getConvidados());
            reserva.getConvidados().clear();
        }
