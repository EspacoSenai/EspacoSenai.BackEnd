...
public class ReservaImpressoraService {
    ...
    public void atualizarTemperatura(TemperaturaDTO temperaturaDTO){
        ReservaImpressora reserva = impressoraRepository.findById(temperaturaDTO.id()).orElseThrow(
                () -> new SemResultadosException("Maquina não achada."));
        if(reserva.getStatusReserva() != StatusReserva3D.LIGADA){}

        reserva.setTemperatura(temperaturaDTO.temperatura());
        impressoraRepository.save(reserva);
    }
